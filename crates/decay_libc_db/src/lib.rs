...
// Generated by `build.rs`: `musl_symbols(cpu)` and `musl_libraries(cpu)`, one
// sorted `&[&str]` per `Cpu` — the candidate names a real `zig cc` link
// resolved for that arch, and the `-lFOO` names that link resolved.
...
/// One `<arch>-linux-<abi>` column of the vendored abilist: the function
/// symbols it exports, and the library names those symbols live in (`m`,
/// `pthread`, `dl`, `rt`, `util`, `resolv`, `c`, `ld` — whatever the
/// abilist's own library table happens to list).
struct GlibcColumn {
...
pub fn has_library(libc: Libc, cpu: Cpu, name: &str) -> bool {
    ...
    match libc {
        ...
        Libc::Musl => musl_libraries(cpu).binary_search(&name).is_ok(),
    }
}
